...
```
```{r}
#pairs(~ LIMIT_BAL + BILL_AMT_SEP + BILL_AMT_AUG +BILL_AMT_JUL+BILL_AMT_JUN +BILL_AMT_MAY+BILL_AMT_APR+PAID_AMT_SEP+ PAID_AMT_AUG+ PAID_AMT_JUL +PAID_AMT_JUN+ PAID_AMT_MAY+ PAID_AMT_APR  + AGE ,data = taiwan_bank, 
...
```{r}
gml <- glm(DEFAULT ~., data=taiwan_bank4)
summary(gml)
```
```{r}
library(car)
...
Varibale REPAY_APR is a categorical variables with the levels: -1=pay duly, 1=payment delay for one month, 2=payment delay for two months, . 8=payment delay for eight months, 8=payment delay for eight months and above. Let's look at real REPAY _APR columns
...
taiwan_bank3$RATIO_SEP<- round(taiwan_bank$PAID_AMT_SEP/taiwan_bank$BILL_AMT_SEP*100)
taiwan_bank3$RATIO_AUG<- round(taiwan_bank$PAID_AMT_AUG/taiwan_bank$BILL_AMT_AUG*100)
taiwan_bank3$RATIO_JUL<- round(taiwan_bank$PAID_AMT_JUL/taiwan_bank$BILL_AMT_JUL*100)
taiwan_bank3$RATIO_JUN<- round(taiwan_bank$PAID_AMT_JUN/taiwan_bank$BILL_AMT_JUN*100)
taiwan_bank3$RATIO_MAY<- round(taiwan_bank$PAID_AMT_JUN/taiwan_bank$BILL_AMT_MAY*100)
taiwan_bank3$RATIO_APR<- round(taiwan_bank$PAID_AMT_JUN/taiwan_bank$BILL_AMT_APR*100)
ratio_paid <- (taiwan_bank3[,14:19])
bank_data_new = cbind(taiwan_data,ratio_paid)
View(bank_data_new)
write.csv(bank_data_new, file = "bank_data_new")
...
write.csv(taiwan_bank3, file = "taiwan_bank3")
#View(taiwan_bank3)
```
```{r}
